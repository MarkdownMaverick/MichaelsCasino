...
void UpdateMainMenu(LobbyState *g, Vector2 mouse)
{
    ...
    if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON))
    {
        if (CheckCollisionPointRec(mouse, rects[0]))
            ...
            SwitchState(g, STATE_LEADERBOARD);
        else if (CheckCollisionPointRec(mouse, rects[5]))
            SwitchState(g, STATE_ACHIEVEMENTS);
    }
}
// ============================================================================
...
void UpdateSettings(LobbyState *g, Vector2 mouse)
{
    ...
    if (IsKeyPressed(KEY_DOWN) || (gamepad >= 0 && XboxBtnPressed(gamepad, 12)))
    {
...
void DrawAccountsManager(const LobbyState *g)
{
    ...
    DrawText("A = Login/Select | B = Menu | Y = Use Insurance ", (int)(CENTER_X - 320.0f), (int)SCREEN_H - 60, 20, GRAY);
}
void UpdateAccountsManager(LobbyState *g, Vector2 mouse)
{
    ...
    if (trigger_action)
    {
        ...
        else
        {
            if (g->accounts[i].is_ai)
            {
                ShowAccountStatus(g, TextFormat("Hi i am %s", g->accounts[i].first_name));
            }
            else
            {
...
void UpdateShop(LobbyState *g, Vector2 mouse)
{
    ...
    if (IsKeyPressed(KEY_RIGHT) || (gamepad >= 0 && XboxBtnPressed(gamepad, 14)))
    {
...
static int CompareRounds(const void *a, const void *b)
{
    const LeaderboardEntry *entryA = (const LeaderboardEntry *)a;
    const LeaderboardEntry *entryB = (const LeaderboardEntry *)b;
    return (entryA->total_rounds - entryB->total_rounds);
}
...
static int WrapRounds(const void *a, const void *b) { return CompareRounds(a, b) * (g_current_asc ? -1 : 1); }
...
void SortLeaderboard(LobbyState *g)
{
    ...
    case 0:
        ...
        break;
    case 1:
        qsort(g->leaderboard, count, sizeof(LeaderboardEntry), WrapRounds);
        break;
    case 2:
...
void DrawLeaderboard(const LobbyState *g)
{
    ...
    DrawText(TextFormat("Rounds%s", (g->leaderboard_sort_mode == 1 ? dir : "")), (int)rects[2].x + 40, (int)rects[2].y + 15, 20, BLACK);
...
void DrawAchievements(const LobbyState *g)
{
    ...
    Rectangle prog_bg = {CENTER_X - 250, 280, 500, 25};
    ...
    DrawText("LB/RB, 1/2, Click: Switch Player | D-Pad/WASD/Arrows: Navigate | B: Back",
...
void UpdateAchievements(LobbyState *g, Vector2 mouse)
{
    ...
    if (IsKeyPressed(KEY_ONE) || XboxBtnPressed(gamepad, 4))
    {
        g->achievement_view_mode = 0;
    }
    if (IsKeyPressed(KEY_TWO) || XboxBtnPressed(gamepad, 5))
    {
        g->achievement_view_mode = 1;
    }
...
void UpdateBettingSetup(LobbyState *g, Vector2 mouse)
{
    ...
    { // P1 bet
        ...
        if (IsKeyPressed(KEY_MINUS) || (gamepad >= 0 && XboxBtnPressed(gamepad, 1)))
        {
    ...
    if (IsKeyPressed(KEY_ENTER) || (gamepad >= 0 && XboxBtnPressed(gamepad, 7)))
    {

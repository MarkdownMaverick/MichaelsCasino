...
{
    if (bet_level == BET_3_TOKENS)
        return 3;
    if (bet_level == BET_2_TOKENS)
        return 2;
    return 1;
}
void UpdateSlotReels(LobbyState *core, SlotReelsState *slot)
{
    ...
    if (slot->state == SLOT_STATE_IDLE || slot->state == SLOT_STATE_SHOW_WIN)
    {
        ...
        if (IsKeyPressed(KEY_TWO))
            slot->bet_level = BET_2_TOKENS;
        if (IsKeyPressed(KEY_THREE))
            slot->bet_level = BET_3_TOKENS;
        // Start spin
        if ((IsKeyPressed(KEY_SPACE) || IsKeyPressed(KEY_ENTER) ||
             (gamepad >= 0 && XboxBtnPressed(gamepad, 0))) &&
